...
def divide_training_subset(frame, train, val, test_meta):
    ...
    if test_meta is not None:
        ...
        sub_val   = frame.iloc[:n_train]
        sub_test  = frame.iloc[n_train:]

    # return ('train', sub_train), ('val', sub_val), ('test', sub_test)
    # return ('train', sub_train), ('val', sub_val)
    return ('train', sub_train), ('val', sub_val), ('test', sub_test)
...
def process_lc2(row, source, meta, unique_classes, **kwargs):
    ...
    observations.columns = ["days", "mag", "magerr", "weights"]
...
def create_dataset(meta_df,
                   source='main-code/data/data/raw_data/ZTF/r-LCs',
                   meta='main-code/data/data/raw_data/ZTF/r-meta',
                   target='main-code/data/data/records/ZTF/r-LCs',
                   n_jobs=1,
                   subsets_frac=(0.75, 0.25),
                   test_subset="",
    ...
    for cls_name, cls_meta in tqdm(cls_groups, total=len(cls_groups)):
        ...
        for subset_name, frame in subsets:
            ...
            os.makedirs(dest, exist_ok=True)
            write_records(frame, dest, max_lcs_per_record, source, meta, unique, n_jobs, **kwargs)
...
def deserialize(sample):
    ...
    context_features = {'label': tf.io.FixedLenFeature([],dtype=tf.int64),
                        ...
                        'meta': tf.io.VarLenFeature(dtype=tf.float32),
    ...
    meta_lc = tf.sparse.to_dense(context['meta'])
    input_dict['meta']   = tf.cast(meta_lc, tf.float32)
...
def sample_lc(sample, max_obs, binary=True):
    ...
    return sequence, input_dict['label'], input_dict['lcid'], input_dict['meta']
...
def get_windows(sample, max_obs, binary=True):
    ...
    meta = tf.tile([input_dict['meta']], [len(splits), 1])

    return splits, y, ids, meta


def mask_sample(x, y , i, meta, msk_prob, rnd_prob, same_prob, max_obs):
    '''
    Pretraining formater
    '''

    with tf.name_scope("mask_sample"):
        # Extract time and magnitude from the input tensor
        try:
            seq_time = tf.slice(x, [0, 0], [-1, 1])
            seq_magn = tf.slice(x, [0, 1], [-1, -1])
        except Exception as e:
            print(f"Error slicing tensor: {e}")
    ...
    if time_steps < max_obs:
        ...
        # Change here: Tile the last dimension of mask_fill to match the shape of seq_magn
        seq_magn   = tf.concat([seq_magn, tf.tile(1-mask_fill, [1, tf.shape(seq_magn)[1]])], 0)
        seq_time   = tf.concat([seq_time, tf.tile(1-mask_fill, [1, tf.shape(seq_time)[1]])], 0)
        orig_magn   = tf.concat([orig_magn, tf.tile(1-mask_fill, [1, tf.shape(orig_magn)[1]])], 0)
...
def pretraining_records(source, batch_size, max_obs=100, msk_frac=0.2,
    ...
    # for tfrecord_file in rec_paths:
