...
from astra.src.scheduler import CustomSchedule, WarmUpCosineDecay
from astra.src.transformer import AstraNet, AstraNet_Distil, contrastive_train, k_distil_training
...
def distil_training(args):
    ...
    hparams = {
                    ...
                    "model_params": {
                        "num_global_views": config['num_global_views'],"num_local_views": config['num_local_views'],
                    ...
                    "training_params": {
                        "epochs": config['epochs'], "patience": config['patience'], "initial_lr": config['initial_lr'], "s_temp": config['s_temp'], 
                        "start_t_temp": config['start_t_temp'], "base_t_temp": config['base_t_temp'], "warmup_epochs_temp": config['warmup_epochs_temp'],
                        "base_ema_m": config['base_ema_m'], "final_ema_m": config['final_ema_m'], "momentum_center": config['momentum_center'],
                        "use_custom_schedule": config['use_custom_schedule'], "warmup_epochs_sch": config['warmup_epochs_sch'],
                        "scheduler_type": config['scheduler_type'], "use_ema_scheduler": config['use_ema_scheduler'], "base_lr": config['base_lr'],
                        "batch_size": config['batch_size'], "num_gpus": config['num_gpus'], "n_training_samples":config['n_training_samples']
                    },
                    "data_params": {
                        "buffer_size": config['buffer_size'], "path_to_read": config['path_to_read'], "path_to_val": config['path_to_val'], "path_to_save": run_log_dir,
                        "num_global_views": config["num_global_views"],"num_local_views": config["num_local_views"],"gv_maxlens": config["gv_maxlens"],
                        "lv_maxlens_list": config["lv_maxlens_list"], "apply_noise_list": config["apply_noise_list"], "noise_levels_list": config["noise_levels_list"], 
                        "apply_binning_list": config["apply_binning_list"], "apply_outlier_list": config["apply_outlier_list"], "bin_widths_list": config["bin_widths_list"], 
                        "drop_rates_list": config["drop_rates_list"]
                    }
                }
    # ===============================================
    #
